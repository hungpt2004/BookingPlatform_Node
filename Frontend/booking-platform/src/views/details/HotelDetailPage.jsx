...
export const HotelDetailPage = () => {
  ...
  const validateTimes = (checkInTime, checkOutTime) => {
    ...
    if (checkOutTotal <= checkInTotal) {
      errors.checkOut =
        "Check-out time must be after check-in time when dates are the same";
    }
  ...
  const handleSearch = async () => {
    ...
    if (numberOfPeople > currentHotel?.capacity) {
      setCapacityError(
        `This hotel only accommodates up to ${currentHotel?.capacity} guests`
      );
      return;
    }
  ...
  const getCurrentHotelDetail = async () => {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/hotel/get-hotel-detail/${id}`
      );
  ...
  const getFeedbackByHotelId = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/feedback/get-feedback-hotel/${id}`
      );
      ...
      {loading ? null : error ? (
        ...
        <>
          <Container fluid className="p-5">
            ...
            <p className="">
              <MdLocationPin style={{ color: "#6499E9" }} />{" "}
              {currentHotel.address} -
              <span className="fw-bold" style={{ color: "#6499E9" }}>
                Vị trí hiện tại
              </span>{" "}
              -
              <span className="fw-bold" style={{ color: "#6499E9" }}>
              ...
              <Col lg={6} md={12}>
                {currentHotel?.images?.length > 0 ? (
                    ...
                    >
                      {loading ? (
                        ...
                        <Image
                          ...
                          onClick={() =>
                            handleShowModalImage(currentHotel.images[0])
                          }
                        />
                      )}
                    </motion.div>
                    ...
                    {currentHotel.images.length > 5 && (
                      <div className="more-images mt-3">
                        ...
                        >
                          ...
                          Xem tất cả {currentHotel.images.length} ảnh
                        </Button>
                      </div>
                    )}
                    <Modal
                  ...
                  <div className="no-images-container p-5 text-center bg-light rounded">
                    ...
                    <p className="text-secondary">Không có ảnh nào</p>
                  </div>
                )}
              </Col>
                  ...
                  <Card.Body>
                              ...
                              <Carousel.Item key={feedback.id}>
                                <div className="review-item p-3">
                                  ...
                                  <div className="reviewer d-flex align-items-center mt-3">
                                    ...
                                    />
                                    <p className="reviewer-name mb-0">
                                      {feedback.user.name}
                                    </p>
                                  </div>
                                </div>
                    ...
                    <div className="hotel-description">
                      ...
                      <p className="description-text">
                        {`Với ${dataFacility[0]} và ${dataFacility[1]}, ${
                          currentHotel.hotelName
                        } tọa lạc ở 
            ...
            <div className="row g-3 justify-content-center">
              ...
              <div className="col-md-3">
                ...
                <input
                  ...
                  className={`form-control mt-2 ${
                    timeErrors.checkIn ? "is-invalid" : ""
                  }`}
                />
                {timeErrors.checkIn && (
                  <div className="invalid-feedback">{timeErrors.checkIn}</div>
                )}
              </div>
              ...
              <div className="col-md-3">
                ...
                <input
                  ...
                  className={`form-control mt-2 ${
                    timeErrors.checkOut ? "is-invalid" : ""
                  }`}
                />
                {timeErrors.checkOut && (
                  <div className="invalid-feedback">{timeErrors.checkOut}</div>
                )}
              </div>
                ...
                <CustomInput
                  ...
                  onChange={(e) => {
                    const value = Math.max(
                      1,
                      Math.min(currentHotel?.capacity || 16, e.target.value)
                    );
              ...
              <div className="row justify-content-center mt-2">
                <div className="col-md-8">
                  {capacityError && (
                    <div className="alert alert-danger">{capacityError}</div>
                  )}
                  {availabilityError && (
          ...
          <p className="fw-bold px-5 mb-1">
            Đánh giá 5 {"(⭐)"} {listFeedback.length}
          ...
          <h4 className="px-5 fw-bold mt-3">
            Khách lưu trú ở đây thích nhất điều gì ?
          </h4>
          <div className="d-flex align-items-center px-5">
            <Card style={{ backgroundColor: "#003b95" }}>
              <Card.Text className="px-3 py-2 text-light text-center">
                {currentHotel.rating}
              </Card.Text>
            </Card>
            <span className="fw-bold px-3 fs-6">
              {" "}
              ▪️ {RatingConsider(currentHotel.rating)} ▪️ {listFeedback.length}{" "}
              đánh giá
            </span>
            <a className="align-content-center text-primary text-decoration-underline cursor-pointer">
              <span>Đọc tất cả các đánh giá</span>
            </a>
          </div>
          <Swiper
            navigation={true}
          ...
          >
            {listFeedback.length > 0 ? (
              ...
              <p className="px-5 alert text-center alert-warning">
                Không có đánh giá nào về {currentHotel.hotelName}
              </p>
            )}
          </Swiper>
        </>
